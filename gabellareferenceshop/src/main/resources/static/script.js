...
class ApiService {
    ...
    async createUser(userData) {
        return await this.request(`${API_CONFIG.endpoints.users}/creer utilisateur`, {
            method: 'POST',
            body: userData
        });
    }
    ...
    if (content) {
        content.innerHTML = `
            ...
            `<i class="fas fa-shoe-prints"></i>`
                ...
                <h3>${product.name}</h3>
                <p style="color: var(--dark-gray); margin: 1rem 0;">${product.description}</p>
                <p style="font-size: 1.5rem; color: var(--primary-color); font-weight: 700; margin: 1rem 0;">${product.price.toFixed(2)}FCFA</p>
